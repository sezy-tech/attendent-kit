...
export default function FaceInput() {
  ...
  const processImage = async (image: string) => {
    ...
    for (const face of faces) {
      ...
      const newLandmarkPoint = Object.entries(face.landmarks!).reduce(
        (result, [key, {position}]) => {
          const newPoint = [
            (position.x - left) / scalingRatio.width,
            (position.y - top) / scalingRatio.height,
          ];
      ...
      if (hasFaceInput) {
        userStore.data.state.user.hasFaceInput == true;
